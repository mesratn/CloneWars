...
public class Zone {
	...
	public Zone (int aDimension, int lvls) {	
		this(aDimension, aDimension, lvls);
	}
	
	public Zone (int xDimension, int yDimension, int lvls) {
		this.oRoom = new Room('X', lvls);
		this.lvl = lvls;
	...
	// inner class to represent a cell
	  ...
	  public void updateGrid() {
	      char backChar = ' ', wallChar = 'X', cellChar = ' ', room = 'O';
	      // fill background
	      ...
	      for (int x = 0; x < gridDimensionX; x ++) {
	          for (int y = 0; y < gridDimensionY; y ++) {
	        	  if (x % 4 == 0 || y % 2 == 0) {
	            	  ...
	            	  int Alea = rand.nextInt(2 - 0 + 1) + 0;
	            	  if ((x == 1 && y == 0)  || (x == (gridDimensionX - 2) && y == (gridDimensionY - 1))) {
	            		  grid[x][y] = room;
	            		  this.oRoom.setType('0');
	            	  }
	            	  else if (Alea == 2 && x != 0 && y != 0 && x != (gridDimensionX - 1) && y != (gridDimensionY - 1)) {
	            		  grid[x][y] = room;
	            		  this.oRoom.setType('0');
	            	  }
	            	  else {
	            		  grid[x][y] = wallChar;
	            		  this.oRoom.setType('X');
	            	  }
	              }
	          }
	      }
